...
WITH prices_salaries_change AS (
    WITH prices_annual_change AS (
        SELECT 
            ...
            lead(sum(tpm.average_prices)) OVER (
                ORDER BY tpm.year_cp
            ) AS avg_price_box_next,
            round(
                100 * (
                    lead(sum(tpm.average_prices)) OVER (
                        ORDER BY tpm.year_cp
    ...
    SELECT 
        ...
        round(
            100 * (
                lead(sum(tpm.average_salary_CZK)) OVER (
                    ORDER BY tpm.year_cp
                ) / sum(tpm.average_salary_CZK)
            ) - 100,
            2
        ) AS annual_change_salaries
    FROM
        t_petr_musil_project_SQL_primary_final tpm
    JOIN 
        prices_annual_change ON tpm.year_cp = prices_annual_change.year_cp
    WHERE 
